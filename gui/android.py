...
from electrum_oracoin import SimpleConfig, Wallet, WalletStorage, format_satoshis
from electrum_oracoin.bitcoin import is_address
...
def pay_to(recipient, amount, label):
    ...
    droid.dialogCreateSpinnerProgress("Electrum-Oracoin", "signing transaction...")
    droid.dialogShow()
...
def receive_loop():
    ...
    while out is None:
        ...
        if not event:
            ...
            amount = modal_input('Amount', 'Amount you want to receive (in ORA). ', format_satoshis(receive_amount) if receive_amount else None, "numberDecimal")
...
def make_bitmap(data):
    ...
    try:
        import qrcode
        from electrum_oracoin import bmp
        qr = qrcode.QRCode()
        qr.add_data(data)
...
class ElectrumGui:
    ...
    def __init__(self, config, _network):
        ...
        if not storage.file_exists:
            ...
            droid.dialogCreateSpinnerProgress("Electrum-Oracoin", msg)
            droid.dialogShow()
