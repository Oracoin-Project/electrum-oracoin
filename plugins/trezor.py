...
from electrum_oracoin.account import BIP32_Account
from electrum_oracoin.bitcoin import EncodeBase58Check, public_key_to_bc_address, bc_address_to_hash_160
from electrum_oracoin.i18n import _
from electrum_oracoin.plugins import BasePlugin, hook, always_hook, run_hook
from electrum_oracoin.transaction import deserialize
from electrum_oracoin.wallet import BIP32_HD_Wallet
from electrum_oracoin.util import print_error
from electrum_oracoin.wallet import pw_decode, bip32_private_derivation, bip32_root
...
class TrezorWallet(BIP32_HD_Wallet):
    ...
    def show_address(self, address):
        ...
        try:
            ...
            give_error(e)
        try:
            self.get_client().get_address('Oracoin', address_n, True)
        except Exception, e:
            give_error(e)
        finally:
    ...
    def sign_message(self, address, message, password):
        ...
        try:
            ...
            give_error(e)
        try:
            msg_sig = self.get_client().sign_message('Oracoin', address_n, message)
        except Exception, e:
            give_error(e)
        finally:
            self.plugin.handler.stop()
    ...
    def sign_transaction(self, tx, password):
        ...
        try:
            signed_tx = self.get_client().sign_tx('Oracoin', inputs, outputs)[1]
        except Exception, e:
            give_error(e)
        finally:
            self.plugin.handler.stop()
